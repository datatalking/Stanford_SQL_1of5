...
/*** Wrap these in a python script for automation
  v1 test that db exists, if not start one
  v2 test that tables exist
  v3 test that script updates n # columns to match n
  select sID, sName, GPA would be three columns
  v4 test the data type of the data before importing
  v5 test and generate report for each table and row
  v6 test code for drift away from mean or if left skew is normal that new data doesn't do 1 std dev different***/
